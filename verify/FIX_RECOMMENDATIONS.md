...
### ✅ 追加 #4 — 01_GENERAL.md line 110–163：存取方式缺 Claude Mobile App

**問題摘要**
「存取方式」主表格列了 6 種管道（Claude.ai、Desktop、CLI、Design、Microsoft 365、API），**獨漏 Claude Mobile App**（iOS + Android）。文章其他地方已多處提到手機（Dispatch 派任務、跨裝置同步、Remote Control 給 CLI），主表卻沒列，自相矛盾。

ASCII「四個入口」圖亦未更新：表格實列 6 項、圖僅畫 4 項，加上手機後落差更大。

驗證報告**未掃到此條**（01_GENERAL 驗證範圍聚焦於事實正確性，結構完整性不屬掃描範疇）。

**獨立查證**（2026-04-22）

[官方下載頁 claude.com/download](https://claude.com/download) 明示支援 Web / Desktop（macOS / Windows / ChromeOS）/ iOS / Android。

Mobile 客觀功能（[官方 Help Center — Claude Mobile apps](https://support.claude.com/en/collections/9387080-claude-mobile-apps) 與多家媒體證實）：
- **2026 iOS / Android 完整 feature parity**
- **Voice Mode**（2025-05 推出、2026 年初所有方案免費）：Buttery / Airy / Mellow / Glassy 四聲音、hands-free 連續聆聽或 push-to-talk
- **Dispatch**（2026-03-17 research preview）：從手機派任務給 Desktop Cowork 或本機 Claude Code CLI，支援定時 / 週期任務
- **相機即拍即問**、**iOS Siri Shortcuts**、**Android 桌面 widgets**、**Maps / iMessage 連動**
- **對話 / Projects / memory / preferences 跨裝置同步**

**決議建議**
三處同改：
1. 存取方式表：在 Claude.ai 後、Desktop 前插入 Mobile App 一行
2. 新增 `### Claude Mobile App 特色功能` 小節（與 Desktop 小節對稱，放在 Desktop 小節後、Microsoft 365 前）
3. 拿掉 ASCII「四個入口」圖（原本已與實列 6 項表格不對齊，補手機後更亂）；改為精簡的下載連結清單
...
| Claude Desktop | 桌面應用程式 | macOS / Windows | ... |
```

並新增 Mobile 特色小節（voice / dispatch / 相機 / 平台整合 / 同步 5 項）；ASCII 圖替換為下載連結分類清單。
