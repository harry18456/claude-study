...
> **推出時間：** 2026-04-20  
> **支援方案：** Pro / Max / Team / Enterprise（Claude web 與 Claude Desktop）

Live Artifacts 是 Cowork 內**可持續更新**的產出物——建一次 dashboard 或 tracker，下次開啟會**自動重新連線拉最新資料**刷新，不用重跑對話。

```
傳統 Artifacts：                    Live Artifacts：
┌──────────────────┐                ┌──────────────────────────┐
│ 產出當下靜態      │                │ 每次開啟自動串連          │
│ 要更新 → 重問     │                │ → 重新拉資料 → 即時刷新   │
└──────────────────┘                └──────────────────────────┘
...
| | 傳統 Artifacts | Live Artifacts |
|---|---|---|
| **資料新鮮度** | 產出當下的快照 | 每次開啟即時刷新 |
| **連線能力** | 無 | 透過 MCP connectors 串接外部服務 |
| **儲存位置** | 對話內 | 獨立 **Live Artifacts 分頁**，保留版本歷史 |
| **跨 session 使用** | 不能 | 任何 session 皆可開啟同一份 |

**原生支援的連接器（常見）：**
- Gmail、Google Calendar、Google Drive
- Slack
- Notion、HubSpot、Jira、Salesforce
- Snowflake、Asana 等 38+ 工具
- 其他資料來源（如 Power BI、BigQuery、內部系統）可透過**第三方 MCP connector** 擴充

**典型使用情境：**

```
「建一個儀表板：
 今天的行事曆活動 + 最近 Gmail + Notion 客戶筆記」

「每天早上 Meta / Google / LINE 廣告成效匯總，
 依客戶分頁顯示預算消耗進度」
```

**使用注意：**

- **Token 消耗**：每次刷新都會重新呼叫 connectors 並重算，串接資料源越多、刷新越頻繁則用量越高
- **資料源限制**：取決於可用的 MCP connectors；內部系統或自建資料庫需自行架設 MCP server
- **平台範圍**：Claude web 與 Claude Desktop 支援完整，手機端功能有限
- **前置設定**：第一次使用前需在 Cowork Settings 預先連好要用的服務

> 官方公告：[Claude on X（2026-04-20）](https://x.com/claudeai/status/2046328619249684989)
