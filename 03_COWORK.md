...
Dispatch 是 Cowork 的**非同步**延伸——你在手機派任，Claude 在桌機執行，你不需要守在電腦前。
...
1. 手機開啟 Claude App → 切換到 Cowork session
2. 根據對話內容自動選擇資料夾
3. 輸入任務指令
4. 桌機上的 Claude Desktop 自動開始執行
...
| **啟動方式** | Claude Desktop Cowork Tab | 終端機 `claude` 指令 |
| **任務性質** | 一般辦公室工作流 | 開發工作流 |
